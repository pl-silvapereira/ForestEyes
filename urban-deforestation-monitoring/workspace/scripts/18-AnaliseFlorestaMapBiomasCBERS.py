...
def main():
    ...
    # Categoria 3 do MapBiomas / Floresta
    ...
    # =========================================================================
    # PASSO 3: Classificação com Base no Shapefile 2023 + Filtro de Nuvens
    # =========================================================================
    print("Passo 3: Rasterizando Shapefile 2023 (Floresta=#006400, Demais=#A9A9A9) e mascarando nuvens...")
    ...
    for _, row in gdf_2023.iterrows():
        ...
        # Categoria 3 ou contendo Floresta = 1, Demais = 2
        if '3' in c_name or 'Floresta' in c_name:
            shapes_class_3.append((row.geometry, 1))
        else:
            shapes_class_3.append((row.geometry, 2))

    raster_class_3 = rasterize(
        shapes_class_3, out_shape=(height, width), transform=cbers_transform, fill=2, dtype=np.uint8
    )

    # Cores: Floresta (#006400 -> [0, 100, 0]), Demais (#A9A9A9 -> [169, 169, 169])
    ...
    for b, val in enumerate([0, 100, 0]):
        img_class_3[b] = np.where(cond_floresta, val, img_class_3[b])
    for b, val in enumerate([169, 169, 169]):
        img_class_3[b] = np.where(cond_demais, val, img_class_3[b])

    # Detectar Nuvens e Sombras na imagem CBERS 2024 para pintar de preto
    ...
    # Aplicar preto onde for nuvem/sombra ou fora da ROI
    for b in range(3):
    ...
    # =========================================================================
    # PASSO 5: Segmentação restrita à área verde (Compara 2023 vs 2024 no CBERS Colorido)
    # =========================================================================
    print("Passo 5: Segmentação nas áreas verdes (Contornos: Amarelo=Floresta [3], Vermelho=Diferente)...")
    ...
    # Executar SLIC apenas onde é a classe verde (cond_floresta)
    ...
    img_seg_transposed = np.moveaxis(rgb_normalized.copy(), 0, -1)
    ...
    for sp_id in ids_unicos:
        ...
        # Verificar maioria em 2024 para definir cor do contorno
        ...
        # Amarelo [255, 255, 0] se for floresta [3], Vermelho [255, 0, 0] se for diferente
        cor_borda = [255, 255, 0] if is_floresta_24 else [255, 0, 0]
    ...
    step5_bands = np.array([
        img_seg_final[0], img_seg_final[1], img_seg_final[2],
    ...
    print(f"-> Salvo em: {path_step5_out}")

    # =========================================================================
    # PASSO 6: Segmentação idêntica ao Passo 5, com fundo em Escala de Cinza / NDVI aproximado
    # =========================================================================
    print("Passo 6: Gerando segmentação com fundo em escala de cinza (simulação NDVI)...")
    
    # Criar fundo em escala de cinza (média das bandas RGB)
    gray_bg = np.mean(rgb_normalized, axis=0).astype(np.uint8)
    gray_stack = np.array([gray_bg, gray_bg, gray_bg], dtype=np.uint8)
    img_gray_transposed = np.moveaxis(gray_stack, 0, -1)
    ...
    for sp_id in ids_unicos:
        ...
        sub_img = img_gray_transposed[slc[0], slc[1]]
        sub_img[borda_grossa] = cor_borda

    img_gray_final = np.moveaxis(img_gray_transposed, -1, 0)
    step6_bands = np.array([
        img_gray_final[0], img_gray_final[1], img_gray_final[2],
        np.where(cond_floresta, 255, 0)
    ], dtype=np.uint8)
    ...
    print(f"-> Salvo em: {path_step6_out}")
    print("\n[SUCESSO] Processo concluído com todas as novas etapas geradas!")
