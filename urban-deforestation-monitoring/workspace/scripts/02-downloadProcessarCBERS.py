...
# Try-except para rodar localmente ou no Colab sem quebrar se a lib não existir no pre-check
try:
...
def baixar_e_processar_cbers():
    ...
    # Tenta ler do argumento (Orquestrador) ou da variável de ambiente (Fallback)
    if len(sys.argv) >= 3:
    ...
    if not os.path.exists(caminho_json):
        print(f"[ERRO CRÍTICO] Relatório JSON não encontrado: {caminho_json}")
        sys.exit(1)
    ...
    print(f"[RELATÓRIO] CBERS gerado: {caminho_json_resultado}\n")
